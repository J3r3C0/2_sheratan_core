...
class WebRelayBridge:
    ...
    def __init__(self, settings: WebRelaySettings):
        ...
        (Path(storage.DATA_DIR).parent / "webrelay_out").mkdir(parents=True, exist_ok=True)
        (Path(storage.DATA_DIR).parent / "webrelay_in").mkdir(parents=True, exist_ok=True)
    ...
    def enqueue_job(self, job_id: str) -> Path:
        ...
        out_dir = Path(storage.DATA_DIR).parent / "webrelay_out"
    ...
    def try_sync_result(self, job_id: str, remove_after_read: bool = True) -> Optional[models.Job]:
        ...
        in_dir = Path(storage.DATA_DIR).parent / "webrelay_in"
